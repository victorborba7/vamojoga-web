...
export default function CollectionDetailPage() {
  ...
  async function handleAddGame() {
    ...
    try {
      ...
      setCollection((a) =>
        a ? { ...a, games: [...a.games, game], game_count: a.game_count + 1 } : a
      );
  ...
  async function handleRemoveGame(gameId: string) {
    ...
    if (!confirm(`Remover "${game?.name ?? "jogo"}" da colecao?`)) return;
    await removeGameFromCollection(collection.id, gameId);
    setCollection((a) =>
      a
        ? {
            ...a,
            games: a.games.filter((g) => g.game_id !== gameId),
            game_count: a.game_count - 1,
          }
        : a
    );
  }
  ...
  async function handleInvite() {
    ...
    try {
      ...
      setCollection((a) =>
        a ? { ...a, members: [...a.members, member], member_count: a.member_count + 1 } : a
      );
  ...
  async function handleRemoveMember(userId: string) {
    ...
    if (!confirm(`Remover "${member?.username ?? "membro"}" da colecao?`)) return;
    await removeMember(collection.id, userId);
    setCollection((a) =>
      a
        ? {
            ...a,
            members: a.members.filter((m) => m.user_id !== userId),
  ...
  async function handleDelete() {
    if (!collection || !confirm(`Excluir "${collection.name}"? Esta acao nao pode ser desfeita.`)) {
      return;
    }
  ...
  if (loading) {
    return (
      <PageContainer>
        <div className="p-4 text-muted text-sm">Carregando...</div>
      </PageContainer>
    );
  }
  ...
  if (!collection) {
    return (
      <PageContainer>
        <div className="p-4 text-muted text-sm">Colecao nao encontrada.</div>
      </PageContainer>
    );
  }
  ...
  return (
    <PageContainer>
      <PageHeader title={collection.name} backHref={routes.collections} />
          ...
          {editing ? (
            ...
            <>
              ...
              <p className="text-xs text-muted">
                {collection.game_count} {collection.game_count === 1 ? "jogo" : "jogos"} · {collection.member_count}{" "}
                {collection.member_count === 1 ? "membro" : "membros"}
              </p>
              {isOwner && (
        ...
        {tab === "games" && (
          <div className="space-y-3">
            <Card className="p-3 space-y-2">
              ...
              <Button size="sm" className="w-full" disabled={!selectedGameId || addingGame} onClick={handleAddGame}>
            ...
            {collection.games.length === 0 ? (
              ...
              <>
                <div className="flex items-center justify-between px-1">
                  <p className="text-xs text-muted">
                    {collection.games.length} {collection.games.length === 1 ? "jogo" : "jogos"}
                  </p>
                  <div className="flex gap-1">
                    <button
                      onClick={() => toggleView("list")}
                      className={`p-1.5 rounded-lg transition-colors ${
                        viewMode === "list" ? "bg-white/10 text-foreground" : "text-muted hover:text-foreground"
                      }`}
                    >
                    ...
                    <button
                      ...
                      className={`p-1.5 rounded-lg transition-colors ${
                        viewMode === "grid" ? "bg-white/10 text-foreground" : "text-muted hover:text-foreground"
                      }`}
                    >
          ...
          <div className="space-y-3">
            {isOwner && (
              <Card className="p-3 space-y-2">
                ...
                />
                <Button size="sm" className="w-full" disabled={!selectedUser || inviting} onClick={handleInvite}>
    ...
    <Card className="p-3 flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-foreground">
          ...
          {isMe && <span className="text-xs text-muted ml-1">(voce)</span>}
        </p>
